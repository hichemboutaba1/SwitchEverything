    ...
    <div className="min-h-screen overflow-x-hidden">
      ...
      <section className="py-16 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/blog" className="text-sm font-medium transition-colors hover:text-white" style={{ color: "var(--accent)" }}>
            All articles →
          </Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {BLOG_POSTS.slice(0, 3).map((post) => (
            ...
            >
              <div className="flex items-center justify-center text-4xl" style={{ height: 90, background: "rgba(108,99,255,0.06)", borderBottom: "1px solid var(--border)" }}>
                {post.coverEmoji}
              </div>
              <div className="p-4 flex-1 flex flex-col">
                <p className="text-xs mb-2" style={{ color: "var(--text-secondary)" }}>{post.readTime} min read</p>
                <h3 className="text-sm font-semibold leading-snug mb-2 line-clamp-2 group-hover:text-white transition-colors" style={{ color: "var(--text-primary)" }}>
                  {post.title}
                </h3>
                <span className="mt-auto text-xs font-medium" style={{ color: "#6c63ff" }}>Read →</span>
              </div>
            </Link>
          ))}
        </div>
